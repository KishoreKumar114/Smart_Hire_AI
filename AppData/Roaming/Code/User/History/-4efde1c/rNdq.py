# app.py - COMPLETE PREMIUM VERSION WITH PERFECT SETTINGS
...
def main():
    ...
    with st.sidebar:
        ...
        current_theme_text = "Dark" if st.session_state.dark_mode else "Light"
        
        st.markdown(f"""
        <div style='text-align: center; padding: 10px; background: rgba(255, 255, 255, 0.1); 
                    border-radius: 10px; margin-bottom: 20px;'>
            <div style='font-size: 1.2rem; margin-bottom: 5px;'>{current_theme_icon}</div>
            <div style='font-size: 0.8rem; opacity: 0.9;'>{current_theme_text} Mode</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_theme1:
            if not st.session_state.dark_mode:
                st.markdown("""
                <div style='text-align: center; padding: 5px; background: rgba(255, 255, 255, 0.2); 
                            border-radius: 5px; margin-bottom: 5px; font-size: 0.7rem;'>
                    Active
                </div>
                """, unsafe_allow_html=True)
            if st.button("☀️", key="sidebar_light", use_container_width=True):
                st.session_state.dark_mode = False
                st.rerun()
        ...
        with col_theme2:
            if st.session_state.dark_mode:
                ...
                """, unsafe_allow_html=True)
            if st.button("🌙", key="sidebar_dark", use_container_width=True):
