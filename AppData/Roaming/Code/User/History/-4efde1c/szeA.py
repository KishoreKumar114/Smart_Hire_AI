...
def show_jd_generator():
    ...
    with col1:
        ...
        department = st.selectbox("Department", ["Engineering", "Data Science", "Product", "Design", "Marketing"], key="jd_dept")
        experience = st.selectbox("Experience Level", ["Entry Level", "Mid Level", "Senior Level", "Lead"], key="jd_exp")
        location = st.selectbox("Work Location", ["Remote", "Hybrid", "On-site"], key="jd_location")
        skills = st.text_area("Required Skills & Technologies", placeholder="Python, Machine Learning, SQL, AWS, Docker...", key="jd_skills")
        company_name = st.text_input("Company Name", value="Your Company", key="jd_company")
...
"""
    ...
    with col2:
        if st.session_state.jd_created and st.session_state.generated_jd:
            ...
            st.text_area("Job Description Content", st.session_state.generated_jd, height=400, label_visibility="collapsed", key="jd_display")
