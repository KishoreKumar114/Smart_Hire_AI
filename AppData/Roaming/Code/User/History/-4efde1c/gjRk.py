...
def load_ultra_premium_css():
    ...
    .nav-button-primary:hover {
        ...
        box-shadow: 0 10px 25px rgba(0, 212, 170, 0.6) !important;
    }
...
def create_custom_navigation():
    """Create custom navigation with larger buttons"""
    pages = {
        "Dashboard": "📊",
        "Resume Screening": "📄", 
        "AI Interview Prep": "🎯",
    ...
    # Create custom navigation buttons using HTML
    for page, icon in pages.items():
        is_active = st.session_state.current_page == page
        button_class = "nav-button-primary" if is_active else "nav-button-secondary"
        
        button_html = f"""
        <div style="margin: 10px 0;">
            <button class="{button_class}" onclick="window.location.href='?page={page}'">
...
def main():
    ...
    # Dark Sidebar with Enhanced Navigation
    with st.sidebar:
        # Logo and Brand - Larger
        st.markdown("""
        <div style='text-align: center; padding: 25px 0;'>
            <div style='font-size: 3rem; background: linear-gradient(135deg, #00D4AA, #0099FF); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 900; margin-bottom: 10px;'>🚀</div>
            <h1 style='margin: 10px 0 5px 0; color: white; font-size: 1.8rem;'>SmartHire AI</h1>
            <p style='margin: 0; color: #E0E0E0; font-size: 1rem;'>Premium Recruitment Platform</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col1:
            st.metric("Active Jobs", "18", delta="+2")
            st.metric("Candidates", "127", delta="+15")
        with col2:
            st.metric("Match Rate", "87%", delta="+3%")
            st.metric("Hiring Time", "16d", delta="-2d")
