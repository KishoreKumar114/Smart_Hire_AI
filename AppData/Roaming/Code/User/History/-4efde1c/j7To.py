...
def create_navigation_button(page_name, icon, current_page):
    """Create a navigation button with active state"""
    is_active = current_page == page_name
    button_class = "nav-button active" if is_active else "nav-button"
    
    button_html = f"""
    <button class="{button_class}" onclick="window.location.href='?page={page_name}'">
        {icon} {page_name}
    </button>
    """
    return button_html
...
def main():
    ...
    <div class="current-page-indicator">
        📍 Currently Viewing: {st.session_state.current_page}
    </div>
    """, unsafe_allow_html=True)
...
"""
    ...
    with col2:
        if st.session_state.jd_created and st.session_state.generated_jd:
            ...
            # JD Output with White Background and Black Text
            st.markdown('<div class="jd-generator-output">', unsafe_allow_html=True)
