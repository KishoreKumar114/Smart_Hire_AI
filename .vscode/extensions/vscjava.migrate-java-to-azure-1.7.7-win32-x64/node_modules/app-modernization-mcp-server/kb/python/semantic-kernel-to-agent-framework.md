...
### **MUST**: Install the ONLY required package `agent-framework` using the projects's package manager

- Package details
    - name: `agent-framework`
    - release state: `prerelease`, so ENSURE prerelease package is allowed, e.g., `--pre` for pip, `--prerelease=allow` for uv
    - it's a meta package contains all agent-framework artifacts and variants, so DON'T add other packages like
        - `agent-framework[azure,openai]`, NOT SUPPORTED
        - `agent-framework-azure-ai`, already included in the meta package
- DO: use the project's package manager to install the package
...
#### Azure Integration
```python
# Azure clients (requires: pip install agent-framework-azure-ai)
...
#### OpenAI Integration
```python
# OpenAI clients (requires: pip install agent-framework[openai])
